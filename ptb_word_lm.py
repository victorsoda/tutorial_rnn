...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, input_):
    ...
    self._input = input_   # "x, y" named as "input_data, target"
    self._rnn_params = None
    self._cell = None
    self.batch_size = input_.batch_size
    self.num_steps = input_.num_steps
    size = config.hidden_size   # "small_config"的模型对应200
    vocab_size = config.vocab_size   # 10000，总单词数

    # 应该是在run每轮(epoch)里的每个step时，会自动产生一个新的shape=(batch_size, num_steps)的"input_data"
    # （reader.py里 range_input_producer的i会自动增加）
    ...
    logits = tf.nn.xw_plus_b(output, softmax_w, softmax_b)  # shape=(batch_size * num_steps, vocab_size)=(400, 10000)
  ...
  def _build_rnn_graph_lstm(self, inputs, config, is_training):
    ...
    with tf.variable_scope("RNN"):
      for time_step in range(self.num_steps):
        ...
        (cell_output, state) = cell(inputs[:, time_step, :], state)  # cell_output.shape = (batch_size, hidden_size)
...
def run_epoch(session, model, eval_op=None, verbose=False):
  ...
  for step in range(model.input.epoch_size):
    ...
    if verbose and step % (model.input.epoch_size // 10) == 10:
      print("%.3f perplexity: %.3f speed: %.0f wps" %   # 第一个数代表当前epoch的完成比例，如0.1代表完成了10%
